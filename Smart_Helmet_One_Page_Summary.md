...
For accuracy, the browser no longer gives a helmet verdict for every small rider in the distance. It first checks whether a motorcycle and rider are large enough in the frame, crops around the nearest usable group, and then runs the helmet model on that closer view. A helmet result needs higher confidence and must repeat across two checks; otherwise it stays uncertain. I also reduced the model input from 416 to 320 pixels and enabled multi-threaded browser inference where supported.

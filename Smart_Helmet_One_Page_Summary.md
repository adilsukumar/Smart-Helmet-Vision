...
What is working now: selecting local videos in the deployed page, browser-side motorcycle/person and helmet inference, rider-to-bike association, an unmarked sample video, full Python video input, tracking, multi-frame confirmation, duplicate-event control, evidence output, five browser rule scenarios and six automated Python tests. The first browser model load is large and the frame rate depends on the device.
